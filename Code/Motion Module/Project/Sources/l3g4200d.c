/** \file iic.c
 * Code borrowed and modified from demo project
*/
...
IIC_ERRORS getRawDataMagnet(Vector3i *raw_data)
{   
  ...
  return iic_read_data(magnet_rd, (uint8_t *)raw_data, sizeof(Vector3i));
}


// Get the raw acceleration data from the sensor
IIC_ERRORS getRawDataAccel(Vector3i *raw_data)
{
  IIC_ERRORS error = NO_ERROR;
  ...
  return iic_read_data(accel_rd, (uint8_t *)raw_data, sizeof(Vector3i));
}


// Get the raw gyro data from the sensor
IIC_ERRORS getRawDataGyro(Vector3i *raw_data)
{
  IIC_ERRORS error = NO_ERROR;
  ...
  return iic_read_data(gyro_rd, (uint8_t *)raw_data, sizeof(Vector3i));
}
