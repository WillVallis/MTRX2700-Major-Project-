...
void track_object (int tol, float *laser_samples, int delay) {
  ...
  if (delay_count <= delay_period && delay_count != 0) {
    ...
    delay_count = 0;
    first_average = average(half_length, &laser_samples[0]);
    second_average = average(half_length, &laser_samples[half_length]);
